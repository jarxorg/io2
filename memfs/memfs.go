...
var (
	...
	_ io2.WriteFileFS  = (*MemFS)(nil)
	_ io2.RemoveFileFS = (*MemFS)(nil)
)
...
func (fsys *MemFS) CreateFile(name string, mode fs.FileMode) (io2.WriterFile, error) {
	fsys.mutex.Lock()
	defer fsys.mutex.Unlock()
...
// MemFile implements fs.File, fs.ReadDirFile and io2.WriterFile.
type MemFile struct {
	fsys       *MemFS
	name       string
	buf        *bytes.Buffer
